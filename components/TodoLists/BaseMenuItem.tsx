...
import { MenuData } from '@nextail/core/lib/types';
...
function BaseMenuItem() {
  const { selectedList, setSelectedList } = useStore();
      ...
      {BaseItems.map((item, index) => (
        <a
          ...
          className={`${styles.base} ${
            selectedList === item.title ? styles.selected : styles.unselected
          }`}
          href={item.link}
          onClick={() => setSelectedList(item.title)}
        >
          {item.icon}
          <span className="mx-4 text-lg">{item.title}</span>
